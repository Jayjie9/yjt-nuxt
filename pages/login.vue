...
// 账号密码登录
const pwdLogin = () => {
  if (!loginAttr.accountName || !loginAttr.pwd) {
...
const captchaLogin = async () => {
  //  输入校验
  if (!loginAttr.phoneNumber || !loginAttr.captcha) {
  ...
  // 调用登录接口
  try {
    const {data} = await api.loginBySms(userInfo)
    if (data.value && data.value.code === 200) {
      ElMessage.success(data.value.message || "登录成功");
      // TODO: 存储用户信息和token
    } else {
      ElMessage.error(data.value?.message || "登录失败，请重试");
    }
  } catch (error) {
                  ...
                  <div class="third-party-icons">
                    <el-tooltip content="微信登录" placement="top">
                      <i class="el-icon-wechat third-party-icon" @click="thirdPartyLogin('微信')">
                        <svg t="1688888888888" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1234" width="32" height="32">
                          <path d="M692.2 352.6c11.3 0 22.4 0.8 33.5 2.1-30.2-140.8-180.6-245.4-352.2-245.4-191.6 0-348.8 130.8-348.8 297.5 0 96.1 52.4 175 139.9 236.2l-35 105.4 122.3-61.2c43.8 8.7 78.8 17.8 122.7 17.8 11 0 21.9-0.5 32.7-1.4-6.8-23.4-10.8-48-10.8-73.4 0-152.9 131.3-277.7 296-277.7zM498.6 247.9c26.4 0 43.8 17.4 43.8 43.9s-17.4 44-43.8 44c-26.4 0-52.8-17.4-52.8-44 0-26.5 26.4-43.9 52.8-43.9zM266 335.8c-26.4 0-53.1-17.4-53.1-44 0-26.5 26.7-43.9 53.1-43.9 26.4 0 43.8 17.4 43.8 43.9 0 26.6-17.4 44-43.8 44z" fill="#00C800" p-id="1235"></path>
                          <path d="M1010.4 620.7c0-139.3-139.3-252.9-296-252.9-165.8 0-296.4 113.5-296.4 252.9 0 139.6 130.6 252.9 296.4 252.9 34.6 0 69.6-8.7 104.3-17.5l95.5 52.5-26.2-87.3c70.1-52.6 122.5-122.5 122.5-200.7z m-392.4-43.9c-17.4 0-34.9-17.4-34.9-35 0-17.4 17.5-35 34.9-35 26.4 0 43.8 17.6 43.8 35 0 17.6-17.4 35-43.8 35z m191.2 0c-17.3 0-34.6-17.4-34.6-35 0-17.4 17.3-35 34.6-35 26.2 0 43.9 17.6 43.9 35 0 17.6-17.7 35-43.9 35z" fill="#00C800" p-id="1236"></path>
                        </svg>
                      </i>
                    </el-tooltip>
                    <el-tooltip content="QQ登录" placement="top">
                      <i class="el-icon-qq third-party-icon" @click="thirdPartyLogin('QQ')">
                        <svg t="1688888888889" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2345" width="32" height="32">
                          <path d="M511.09761 957.257c-80.159 0-153.737-25.019-201.11-62.386-24.057 6.702-54.831 17.489-74.252 30.864-16.617 11.439-14.546 23.106-11.55 27.816 13.15 20.689 225.583 13.211 286.912 6.767v-3.061z" fill="#1296db" p-id="2346"></path>
                          <path d="M496.65061 957.257c80.157 0 153.737-25.019 201.11-62.386 24.057 6.702 54.83 17.489 74.253 30.864 16.616 11.439 14.543 23.106 11.55 27.816-13.15 20.689-225.584 13.211-286.914 6.767v-3.061z" fill="#1296db" p-id="2347"></path>
                          <path d="M497.12861 474.524c131.934-0.876 237.669-25.783 273.497-35.34 8.541-2.28 13.11-6.364 13.11-6.364 0.03-1.172 0.542-20.952 0.542-31.155C784.27761 229.833 701.12561 57.173 496.64061 57.162 292.15661 57.173 209.00061 229.832 209.00061 401.665c0 10.203 0.516 29.983 0.547 31.155 0 0 3.717 3.821 10.529 5.67 33.078 8.98 140.803 35.139 276.08 36.034h0.972z" fill="#1296db" p-id="2348"></path>
                          <path d="M860.28261 619.782c-8.12-26.086-19.204-56.506-30.427-85.72 0 0-6.456-0.795-9.718 0.148-100.71 29.205-222.773 47.818-315.792 46.695h-0.962C410.88561 582.017 289.65061 563.617 189.27961 534.698 185.44461 533.595 177.87261 534.063 177.87261 534.063 166.64961 563.276 155.56661 593.696 147.44761 619.782 108.72961 744.168 121.27261 795.644 130.82461 796.798c20.496 2.474 79.78-93.637 79.78-93.637 0 97.66 88.324 247.617 290.576 248.996a718.01 718.01 0 0 1 5.367 0C708.80161 950.778 797.12261 800.822 797.12261 703.162c0 0 59.284 96.111 79.783 93.637 9.55-1.154 22.093-52.63-16.623-177.017" fill="#1296db" p-id="2349"></path>
                        </svg>
                      </i>
                    </el-tooltip>
                    <el-tooltip content="支付宝登录" placement="top">
                      <i class="el-icon-alipay third-party-icon" @click="thirdPartyLogin('支付宝')">
                        <svg t="1688888888890" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3456" width="32" height="32">
                          <path d="M230.4 576.512v-21.504h142.336c5.12 40.96 17.408 81.92 35.84 117.76-53.248 30.72-99.328 51.2-164.864 71.68 0 0-71.68 35.84-35.84 81.92s129.024 10.24 194.56-25.6c35.84-20.48 76.8-51.2 112.64-81.92 40.96 40.96 92.16 76.8 148.48 102.4 158.72 76.8 184.32-40.96 119.808-92.16-71.68-56.32-158.72-81.92-225.28-89.088-25.6-28.672-46.08-66.56-61.44-107.52H752.64v-56.32H476.16v-61.44h276.48v-30.72H476.16v-88.064h-107.52v88.064H102.4v30.72h266.24v61.44H153.6v56.32l76.8-5.12z m286.72 107.52c61.44 12.288 122.88 43.008 163.84 76.8-40.96 35.84-81.92 61.44-117.76 71.68-51.2 12.288-92.16 10.24-107.52 0 0 0-20.48-10.24 0-20.48 15.36-15.36 35.84-20.48 35.84-20.48 28.672-15.36 56.32-30.72 79.872-51.2 15.36-15.36 33.792-35.84 45.728-56.32z" fill="#1677FF" p-id="3457"></path>
                        </svg>
                      </i>
                    </el-tooltip>
                    <el-tooltip content="新浪微博登录" placement="top">
                      <i class="el-icon-weibo third-party-icon" @click="thirdPartyLogin('微博')">
                        <svg t="1688888888891" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4567" width="32" height="32">
                          <path d="M851.4 590.193c-22.196-66.233-90.385-90.422-105.912-91.863-15.523-1.442-29.593-9.94-19.295-27.505 10.302-17.566 29.304-68.684-7.248-104.681-36.564-36.14-116.512-22.462-173.094 0.866-56.434 23.327-53.39 7.055-51.65-8.925 1.89-16.848 32.355-111.02-60.791-122.395C311.395 220.86 154.85 370.754 99.572 457.15 16 587.607 29.208 675.873 29.208 675.873h0.58c10.009 121.819 190.787 218.869 412.328 218.869 190.5 0 350.961-71.853 398.402-169.478 0 0 0.143-0.433 0.575-1.156 4.938-10.506 8.71-21.168 11.035-32.254 6.668-26.205 11.755-64.215-0.728-101.66z m-436.7 251.27c-157.71 0-285.674-84.095-285.674-187.768 0-103.671 127.82-187.76 285.674-187.76 157.705 0 285.673 84.089 285.673 187.76 0 103.815-127.968 187.768-285.673 187.768z" fill="#E71F19" p-id="4568"></path>
                          <path d="M803.096 425.327c2.896 1.298 5.945 1.869 8.994 1.869 8.993 0 17.7-5.328 21.323-14.112 5.95-13.964 8.993-28.793 8.993-44.205 0-62.488-51.208-113.321-114.181-113.321-15.379 0-30.32 3.022-44.396 8.926-11.755 4.896-17.263 18.432-12.335 30.24 4.933 11.662 18.572 17.134 30.465 12.238 8.419-3.46 17.268-5.33 26.41-5.33 37.431 0 67.752 30.241 67.752 67.247 0 9.068-1.735 17.857-5.369 26.202a22.832 22.832 0 0 0 12.335 30.236l0.01 0.01z" fill="#F5AA15" p-id="4569"></path>
                          <path d="M726.922 114.157c-25.969 0-51.65 3.744-76.315 10.942-18.423 5.472-28.868 24.622-23.5 42.91 5.655 18.287 24.752 28.718 43.46 23.217 17.985-5.47 36.564-8.346 56.355-8.346 103.98 0 186.053 82.497 186.053 184.51 0 18.863-3.171 37.06-9.253 54.772-5.655 18.287 4.933 37.588 23.356 43.34a35.64 35.64 0 0 0 10.368 1.442c14.62 0 28.236-8.346 34.458-21.98 9.108-26.48 14.041-54.056 14.041-83.465 0.29-138.87-111.539-247.342-259.023-247.342z" fill="#F5AA15" p-id="4570"></path>
                          <path d="M388.294 534.47c-84.151 0-152.34 59.178-152.34 132.334 0 73.141 68.189 132.328 152.34 132.328 84.148 0 152.337-59.182 152.337-132.328 0-73.15-68.19-132.334-152.337-132.334zM338.53 752.763c-29.454 0-53.39-23.755-53.39-52.987 0-29.228 23.941-52.989 53.39-52.989 29.453 0 53.39 23.76 53.39 52.989 0 29.227-23.937 52.987-53.39 52.987z m99.82-95.465c-6.382 11.086-19.296 15.696-28.726 10.219-9.43-5.323-11.75-18.717-5.37-29.803 6.386-11.09 19.297-15.7 28.725-10.224 9.43 5.472 11.755 18.866 5.37 29.808z" fill="#040000" p-id="4571"></path>
                        </svg>
                      </i>
                    </el-tooltip>
                  </div>
